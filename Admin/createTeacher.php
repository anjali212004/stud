...
{
    $SubjectQuery = mysqli_query($conn, "SELECT Id FROM tblsubject WHERE Subject = '$Subject'");
...
$firstName=$_POST['firstName'];
  ...
  $dateCreated = date("Y-m-d");
    ...
    $query=mysqli_query($conn,"INSERT into tblteacher(firstName,lastName,emailAddress,password,SubjectId,classId,classArmId,dateCreated) 
    value('$firstName','$lastName','$emailAddress','$sampPass_2','$SubjectId','$classId','$classArmId','$dateCreated')");
      ...
      try {
          ...
          foreach ($data as $row) {
              if (!empty($row['0']) && !empty($row['1']) && !empty($row['2']) && !empty($row['3']) && !empty($row['4'])) {
                  ...
                  $className = $row['4'];
                  $classArmName = $row['5'];
                  $SubjectId = getSubjectId($conn, $Subject);
                  $classId = getClassId($conn, $className);
                  $classArmId = getClassArmId($conn, $classArmName);
                  

                  $insert_query = mysqli_query($conn, "INSERT into tblteacher(firstName,lastName,emailAddress,password,SubjectId,classId,classArmId,dateCreated) 
                  value('$firstName','$lastName','$emailAddress','$sampPass_2','$SubjectId','$classId','$classArmId','$dateCreated')");
 ...
 if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit")
	{
        ...
        if(isset($_POST['update'])){
             ...
             $firstName=$_POST['firstName'];
              ...
              $phoneNo=$_POST['SubjectId'];
              $classId=$_POST['classId'];
              $classArmId=$_POST['classArmId'];
              $dateCreated = date("Y-m-d");

    $query=mysqli_query($conn,"update tblteacher set firstName='$firstName', lastName='$lastName',
    emailAddress='$emailAddress', password='$password',SubjectId='$SubjectId', classId='$classId',classArmId='$classArmId'
    where Id='$Id'");
            if ($query) {
            ...
            <div class="col-lg-12">
                   ...
                   <div class="form-group row mb-3">
                     ...
                     <div class="form-group row mb-3">
                        ...
                        <label class="form-control-label">Select Subject<span class="text-danger ml-2">*</span></label>
                         <?php
                        $qry= "SELECT * FROM tblsubject ORDER BY Subject ASC";
                        $result = $conn->query($qry);
                        $num = $result->num_rows;		
                        if ($num > 0){
                          echo ' <select required name="SubjectId" onchange="classArmDropdown(this.value)" class="form-control mb-3">';
                          echo'<option value="">--Select Subject--</option>';
                          while ($rows = $result->fetch_assoc()){
                          echo'<option value="'.$rows['Id'].'" >'.$rows['Subject'].'</option>';
                              }
                                  echo '</select>';
                              }
                            ?>  
              ...
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3">
                  ...
                  <?php
                      $query = "SELECT tblteacher.Id,tblclass.className,tblclassarms.classArmName,tblclassarms.Id AS classArmId,tblteacher.firstName,
                      tblteacher.lastName,tblteacher.emailAddress,tblsubject.Subject,tblteacher.dateCreated
                      FROM tblteacher
                      INNER JOIN tblsubject ON tblsubject.Id = tblteacher.StudentId
